...
export default class extends Controller {
    ...
    static classes = ['loadingSpinner', 'soundButton', 'hidden', 'sentenceList', 'sentenceItem']
    ...
    async fetchSentences(query) {
        ...
        return fetch(`${this.sentencesUrlValue}&${queryString}`)
                .then(response => {
                    if (!response.ok) {
    ...
    async searchSentences({ currentTarget, params: { query } }) {
        ...
        if (!container instanceof HTMLDivElement || !container) {
        ...
        sentences.forEach((sentence) => {
            ...
            if (sentence?.audioUrl) {
                const audioElement = this.createAudioComponentElement(sentence?.audioUrl);
    ...
    createSentenceItemElement( {text, author} ) {
        ...
        sentenceItem.innerHTML = `<div class="flow">
                                    <p>${text}</p>
                                    <em class="author">Author: ${author}</em>
                               </div>`;
